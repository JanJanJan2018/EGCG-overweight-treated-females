...
MI_2_ow <- MI_2_healthy %>% mutate(Fold_Change_MI_Males_to_Overweight_Females=MI_Male_Means/preEGCG_Means)

ow_2_healthy <- MI_2_ow %>% mutate(Fold_Change_Overweight_Females_to_Healthy_Males=preEGCG_Means/HealthyMale_Means)

MI_2_eg <- ow_2_healthy %>% mutate(Fold_Change_MI_Males_to_Post_EGCG_Females=MI_Male_Means/Post_EGCG_Means)

row.names(MI_2_eg) <- row.names(BothStudies)
```
...
```{r}
write.csv(MI_2_eg, 'fold_change_5_groups.csv', row.names=TRUE)
```

Read in the above file if you haven't already or if you are coming back to this file.
```{r}
MI_2_eg <- read.csv('fold_change_5_groups.csv', sep=',', row.names=1)
```
...
```{r}
Top5_EGCG <- MI_2_eg[order(MI_2_eg$Fold_Change_Post_EGCG_to_Overweight_Females,decreasing=TRUE)[1:5],6:10]
...
```{r}
Least5_EGCG <- MI_2_eg[order(MI_2_eg$Fold_Change_Post_EGCG_to_Overweight_Females,decreasing=FALSE)[1:5],6:10]
row.names(Least5_EGCG) <- row.names(MI_2_eg[order(MI_2_eg$Fold_Change_Post_EGCG_to_Overweight_Females,decreasing=FALSE),])[1:5]
...
```{r}
Top5_MI <- MI_2_eg[order(MI_2_eg$Fold_Change_MI_Males_to_Healthy_Males,decreasing=TRUE)[1:5],6:10]
row.names(Top5_MI) <- row.names(MI_2_eg[order(MI_2_eg$Fold_Change_MI_Males_to_Healthy_Males, decreasing=TRUE),])[1:5]
row.names(Top5_MI)
```
...
```{r}
Least5_MI <- MI_2_eg[order(MI_2_eg$Fold_Change_MI_Males_to_Healthy_Males,decreasing=FALSE)[1:5],6:10]
row.names(Least5_MI) <- row.names(MI_2_eg[order(MI_2_eg$Fold_Change_MI_Males_to_Healthy_Males, decreasing=FALSE),])[1:5]
row.names(Least5_MI)
...
```{r}
Top5_MI_fem <- MI_2_eg[order(MI_2_eg$Fold_Change_MI_Males_to_Overweight_Females,decreasing=TRUE)[1:5],6:10]
row.names(Top5_MI_fem) <- row.names(MI_2_eg[order(MI_2_eg$Fold_Change_MI_Males_to_Overweight_Females, decreasing=TRUE),])[1:5]
...
```
```{r}
ggplot(Top5_MI_fem_t, aes(x = Top5_MI_fem_t$MIR548AD, y = row.names(Top5_MI_fem_t)))+ 
      geom_point() + 
  ylab(label="Fold Change Subsets") + 
  xlab("The Most Fold Change Gene, MIR548AD")


```

The gene, MIR548AD, that is the most expressed in males with MI compared to overweight females is 488% or almost five fold more expressed in males with MI than overweight females. But only 80% over expressed in healthy males when compared to overweight females. This gene is also only 1% under expressed in males with MI compared to healthy males.Overweight females using EGCG show an 8% over expression in this gene when compared to overweight females not taking EGCG.
...
```
```{r}
ggplot(Least5_MI_fem_t, aes(x = Least5_MI_fem_t$CA1, y = row.names(Least5_MI_fem_t)))+ 
      geom_point() + 
  ylab(label="Fold Change Subsets") + 
  xlab("The Least Fold Change Gene, CA1")


```

The gene CA1 is least expressed in males with MI compared to overweight females by 73%. This gene is only slightly expressed more in males with MI compared to healthy males by 2% and only slightly under expressed in overweight females using EGCG compared to overweight females not using EGCG by 3%. There is a huge over expressing of this gene in overweight females not using EGCG compared to healthy males by more than 3 3/4 folds or 375% of an increase in gene expression.
...
This given summary above for this gene could mean many things when observing that it is overexpressed in overweight females more than three times the amount of healthy males and 75% more than males with MI. The changes in expression are almost negligible within each subcategory of gender comparisons. Overweight females might be expressing a lot of this gene because they have more gastric acid, or bone resorption of calcium, or have more saliva, create more cerebrospinal fluid, have more carbon dioxide in their blood that needs to be put through reversible hydration. Since the overweight females using EGCG show decreased gene expression levels of this gene by 3% compared to those overweight females not using EGCG, this could mean that EGCG lowers the need for this gene in those biological processes. 



What genes were the most expressed when comparing overweight females treated with EGCG to healthy males?
```{r}
row.names(MI_2_eg[order(MI_2_eg$Fold_Change_Overweight_Females_to_Healthy_Males,decreasing=TRUE),])[1:5]
...
```{r}
row.names(MI_2_eg[order(MI_2_eg$Fold_Change_Overweight_Females_to_Healthy_Males,decreasing=FALSE),])[1:5]
...
```{r}
row.names(MI_2_eg[order(MI_2_eg$Fold_Change_MI_Males_to_Post_EGCG_Females,decreasing=TRUE),])[1:5]
...
```{r}
row.names(MI_2_eg[order(MI_2_eg$Fold_Change_MI_Males_to_Post_EGCG_Females,decreasing=FALSE),])[1:5]
